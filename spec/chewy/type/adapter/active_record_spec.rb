...
describe Chewy::Type::Adapter::ActiveRecord, :active_record do
  ...
  describe '#import' do
    ...
    context 'default scope' do
      ...
      specify { import.should == [{index: cities.first(3)}] }

      specify { import(City.order(:id)).should == [{index: cities.first(3)}] }
      specify { import(City.order(:id), batch_size: 2)
        .should == [{index: cities.first(2)}, {index: [cities[2]]}] }

      specify { import(cities).should == [{index: cities}] }
      specify { import(cities, batch_size: 3)
        .should == [{index: cities.first(3)}, {index: cities.last(1)}] }

      specify { import(cities.map(&:id))
        .should == [{index: cities.first(3)}, {delete: [cities.last.id]}] }
      specify { import(cities.map(&:id), batch_size: 2)
        .should == [{index: cities.first(2)}, {index: [cities[2]]}, {delete: [cities.last.id]}] }
      specify { import(cities.map(&:id), deleted.map(&:id))
        .should == [{index: cities.first(3)}, {delete: [cities.last.id] + deleted.map(&:id)}] }
      specify { import(cities.map(&:id), deleted.map(&:id), batch_size: 3).should == [
